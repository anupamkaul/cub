...
void TAB_enter(table t, symbol s, bvalue b)
{
	...
	e->s = s;
	e->b = b;
	printf("Name, inserted = %s (or symbol 0x%x)\n", e->s->name, e->s);
...
bvalue TAB_lookup(table t, symbol s)
{
	...
	while(d != NULL)
	{
		...
		if(d->s == s)
		{
		printf("LOOKUP, symbol found 0x%x for name %s\n", d->s, d->s->name);
		return d->b;
		}
		d=d->next;
	}
	...
	printf("Wrong Index Calculation (or Symbol not Found): 0x%x !\n", s);
	//exit(0);
}
...
void TAB_desupdate(table t, symbol s)
{
	...
	{
	printf("WARNING !! REMOVING A SYMBOL (0x%x) THAT DOES NOT EXIST !!\n", s);
	}
}
